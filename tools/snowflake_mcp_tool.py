# tools/snowflake_mcp_tool.py

import os
import httpx
import json
from agno.tools import tool

class SnowflakeMCPTool(Tool):
    """
    A tool to connect to a Snowflake Master Control Program (MCP)
    server via a Server-Sent Events (SSE) URL, send a query,
    and process the streamed response.
    """
    def __init__(self):
        super().__init__(
            name="query_snowflake_mcp",
            description="Use this tool to ask questions or send queries to the Snowflake MCP server. The server will stream back a response.",
        )
        # Get the SSE URL from environment variables for security and flexibility.
        self.sse_url = os.getenv("SNOWFLAKE_SSE_URL")
        if not self.sse_url:
            raise ValueError("SNOWFLAKE_SSE_URL environment variable not set.")

    async def run_async(self, query: str) -> str:
        """
        Connects to the SSE endpoint, sends the query, and aggregates the response.
        This is the method the Agno agent will call.
        """
        if not self.sse_url:
            return "Error: SNOWFLAKE_SSE_URL is not configured."

        aggregated_response = ""
        
        try:
            # httpx is used for making async HTTP requests.
            # We use a POST request to send our query in the body.
            async with httpx.AsyncClient() as client:
                async with client.stream("POST", self.sse_url, json={"query": query}, timeout=120) as response:
                    # Check for successful connection
                    if response.status_code != 200:
                        return f"Error: Failed to connect to Snowflake MCP. Status: {response.status_code}"

                    # Process the SSE stream
                    async for line in response.aiter_lines():
                        # SSE messages start with "data: "
                        if line.startswith("data:"):
                            try:
                                # Remove "data: " prefix and parse the JSON content
                                content = line[5:].strip()
                                data = json.loads(content)
                                # Extract the actual message from the JSON payload
                                if "data" in data:
                                    aggregated_response += data["data"]
                                elif "error" in data:
                                    return f"Error from MCP server: {data['error']}"
                            except json.JSONDecodeError:
                                # Handle cases where the data is not valid JSON
                                print(f"Warning: Could not decode JSON from line: {line}")
                                continue
        except httpx.RequestError as e:
            return f"Error: Could not connect to the Snowflake MCP server at {self.sse_url}. Details: {e}"
        except Exception as e:
            return f"An unexpected error occurred: {e}"

        return aggregated_response if aggregated_response else "Received an empty response from the server."

